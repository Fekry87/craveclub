...
- القياس / Measurements (2026-09-18): a coach records **stroke + distance + time** per swimmer during a session. **There is no options table** — a stroke is a `SWIM_TYPE` skill and a distance is a `DISTANCE` skill (new `SkillType` case; its meters live in `skills.numeric_value`, null for every other type and cleared by the server if the type changes). The manager adds both on the portal's Skills page. `measurements` (`club_id, session_id, swimmer_id, stroke_skill_id, distance_skill_id, time_seconds decimal(7,2), recorded_by`) has no uniqueness — a swimmer can have many in one session. `MeasurementController`, all behind `feature:skills`: coach `GET /coach/measurement-options` (`{ strokes[], distances[] }`, distances shortest first — what the app's pickers read), `GET|POST /coach/sessions/{id}/measurements`, `DELETE …/measurements/{id}` (only the coach who recorded it), `GET /coach/swimmers/{id}/measurements` (paginated history, swimmer must be in the coach's groups → else 403) and the manager's `GET /club/swimmers/{id}/measurements`. Session ownership is the same `whereIn('group_id', coach's groups)->findOrFail` as every coach session route, so **another coach's session is a 404, not a 403**. `store` also refuses (422) a swimmer who is not on the session's effective roster, a session that is not `Live`/`Completed`, and a skill of the wrong type or another club. The skill FKs are `restrict`: `skillDestroy` answers 422 "used by N recorded measurements" instead of the FK's 500, and `skillUpdate` refuses to change the type of a skill that has measurements. **Swimmer side (2026-09-18)**: `GET /swimmer/sessions/{id}` carries `my_measurements` (this swimmer's times in that session, oldest first), and `GET /swimmer/measurements` (`feature:skills`) lists their times as **one entry per training day**, newest first — `{ data: [{ date, count, measurements: [...] }], current_page, last_page, total }`, paginated **by day** so a day is never split; the day is the *session's* date, not the row's `created_at`. Swimmer rows go through `MeasurementController::swimmerRow()` and never include `recorded_by`. `GET /swimmer/measurements/progress?period=day|week|month` (default week; `feature:skills`) feeds the app's progress chart: `{ period, strokes: [{id,name}], points: [{ start, entries: [{stroke_id, count, avg_pace}] }] }` — points ascending, `start` is the bucket (the day, the ISO-Monday week, or the first of the month), and `avg_pace` is the bucket's average **pace per 50m** (`time/meters x 50`), per stroke, so mixed distances average honestly; bucketing is in PHP (SQLite and MySQL date functions differ). Portal: `components/MeasurementHistory.jsx` (read-only table on the coach's SwimmerDetail; hides itself on 403). Recording happens in the mobile app. Tests: `MeasurementTest`
- **Award titles are the club's own (2026-09-18)**: no longer the fixed Day/Week/Month. `award_types` (`club_id, name, xp_value, position`) holds each club's titles; a club is seeded the three defaults (Man of the Day 50 / Week 150 / Month 400) on creation (`Club::booted` → `AwardType::seedDefaults`; the migration seeded existing clubs from their old `leaderboard_settings.award_*_xp`). Manager CRUD at `/club/award-types` (index/store/update/destroy, `feature:leaderboard`); coach reads `/coach/award-types` to pick from. Giving an award now takes `award_type_id` (was `award_type`); `swimmer_awards` gained `award_type_id` (nullable, **set null on delete** so a removed title never breaks past awards) + `award_name` (snapshot, alongside the existing `xp_value` snapshot). **XP is unchanged** — it still sums `swimmer_awards.xp_value`. The award payload (`present()`, `SwimmerAwarded` event, pending/recent) now carries `award_name` instead of `award_type`; the legacy `award_type` column stays nullable for the pre-migration rows. Portal Leaderboard settings replaced the three fixed Award XP inputs with an add/rename/remove list. App: `AwardSheet` fetches the titles, one trophy icon for all (`utils/awards.ts` `AWARD_ICON`). Tests: `AwardTypeTest`, `SwimmerAwardTest`
