...
- Procfile `reverb:` must bind Railway's injected `$PORT` (`--port=${PORT:-${REVERB_PORT:-8080}}`), the same way `web:` does. Binding a fixed 8080 means Railway's edge cannot reach the process and every WebSocket returns a `server: railway-hikari` 500 ("Application failed to respond") that looks like a client bug.
- `frontend/.env.production` deliberately does NOT contain `VITE_REVERB_HOST` / `VITE_REVERB_APP_KEY` — set them as env vars on the Railway frontend service (Vite reads any `VITE_`-prefixed build-time variable). Committing placeholders is what caused the original silent failure.
