...
export default function Groups() {
  ...
  const columns = [
    ...
    { key: 'swimmers', label: 'Swimmers', render: r => (
  ...
  return (
    <div>
      <PageHeader title="Groups" search={search} onSearch={setSearch} searchPlaceholder="Search groups...">
      ...
      <DataTable columns={columns} data={groups} onEdit={handleEdit} onDelete={handleDelete}
        actions={(row) => <Button variant="secondary" onClick={() => openMembers(row)}>Members</Button>}
            ...
            <MobileCardWrapper key={row.id} index={i} accentColor={ac.accent}>
              ...
              <div style={{ background: 'rgba(6,13,31,0.3)', borderRadius: 12, padding: '2px 14px', border: '1px solid rgba(51,65,85,0.1)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0' }}>
                  ...
                  <span style={{ color: row.coach?.name ? '#cbd5e1' : '#475569', fontSize: '0.8125rem', fontStyle: row.coach?.name ? 'normal' : 'italic' }}>{row.coach?.name || 'Unassigned'}</span>
                </div>
              </div>
      ...
      {showMembers && (
        <Modal title={`Members: ${showMembers.name}`} onClose={() => setShowMembers(null)}
          ...
          <ModalActions>
            <Button variant="secondary" onClick={() => setShowMembers(null)}>Cancel</Button>
