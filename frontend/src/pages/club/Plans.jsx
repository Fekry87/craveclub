...
export default function Plans() {
  ...
  const columns = [
    ...
    { key: 'level', label: 'Level', render: r => r.level ? (
      <Badge variant="neutral" label={r.level} />
  ...
  if (showModal) {
    return (
      <FormPage title={editId ? 'Edit Plan' : 'New Plan'} onBack={closeForm}
        icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1D1D1F" strokeWidth="1.8" strokeLinecap="round"><path d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>}>
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h4 style={{ ...labelStyle, margin: 0 }}>Plan Items</h4>
            <Button variant="secondary" size="sm" onClick={() => setForm({ ...form, items: [...form.items, { ...emptyItem }] })}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>
              Add Item
            </Button>
          </div>
          {form.items.map((item, idx) => (
            <div key={idx} className="plan-item-grid" style={{ borderRadius: 16,
              background: '#FFFFFF', padding: 14, marginBottom: 10,
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <Button variant="danger" size="sm" onClick={() => setForm({ ...form, items: form.items.filter((_, i) => i !== idx) })}>
